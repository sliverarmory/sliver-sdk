...
pub extern "C" fn starts(
    ...
    ob.send_output(format!("second arg: {}", dp.get_string().unwrap()).as_str());

    // call do_stuff from myextension.rs
    let result = match {{.ExtensionName}}::do_stuff(firt_arg, dp.get_string().unwrap().as_str()) {
        Ok(v) => v,
        Err(e) => {
